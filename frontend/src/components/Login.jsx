  ...
  return (
    <div>
      <div className="title flex flex-col items-center">
        <h1 className='text-sm xl:text-2xl font-semibold'>Login</h1>
        ...
        <div className="form m-4 text-sm flex flex-col items-center gap-4">
          ...
          <button disabled={isLoading} onClick={handleSubmit} className='rounded bg-blue-700 px-4 py-1 hover:bg-blue-900'>Submit</button>
        </div>
        <div className="firebase-login-options flex justify-center items-center gap-4 m-4">
          <button onClick={handleGoogleLogin} disabled={isLoading} className={`rounded  px-4 py-1 bg-green-600 hover:bg-green-700 ${isLoading && "cursor-not-allowed"}`}>Google</button>
        </div>
    </div>
  )
}
